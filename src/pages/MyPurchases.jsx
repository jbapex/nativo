...
export default function MyPurchases() {
  ...
  useEffect(() => {
    filterOrders();
  }, [orders, searchTerm, statusFilter, activeTab, userStore]);
  ...
  const filterOrders = () => {
    ...
    // Filtrar apenas compras (pedidos onde o usuário é o comprador e NÃO é da loja dele)
    if (user && userStore) {
      ...
      // Se não tem loja, mostrar todos os pedidos do usuário
      filtered = filtered.filter(order => order && order.user_id === user.id);
    } else {
      filtered = [];
    }
    ...
    // Filtrar por busca (nome do produto, ID do pedido, nome da loja)
    if (searchTerm) {
  ...
  const getStatusCounts = () => {
    ...
    let purchasesOrders = orders;
    if (userStore) {
      purchasesOrders = orders.filter(o => 
        o && o.user_id === user.id && o.store_id !== userStore.id
      );
    } else {
      purchasesOrders = orders.filter(o => o && o.user_id === user.id);
    }
    
    return {
      all: purchasesOrders.length,
      pending: purchasesOrders.filter(o => o && o.status === 'pending').length,
      confirmed: purchasesOrders.filter(o => o && o.status === 'confirmed').length,
      processing: purchasesOrders.filter(o => o && o.status === 'processing').length,
      shipped: purchasesOrders.filter(o => o && o.status === 'shipped').length,
      delivered: purchasesOrders.filter(o => o && o.status === 'delivered').length,
      cancelled: purchasesOrders.filter(o => o && o.status === 'cancelled').length,
    };
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Acompanhe os pedidos que você fez em outras lojas
          </p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <Input
                  placeholder="Buscar por ID, produto ou loja..."
                  value={searchTerm}
            ...
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600">
                {searchTerm || statusFilter !== "all" || activeTab !== "all"
                  ...
                  : "Você ainda não fez nenhuma compra em outras lojas"
                }
              </p>
            </CardContent>
          </Card>
